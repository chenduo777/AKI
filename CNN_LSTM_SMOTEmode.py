...
def oldData():
    ...
    model = Sequential()
    model.add(Conv1D(filters = filters,kernel_size = kernel_size, activation='relu', strides=1,input_shape = (X_train.shape[1],1)))
    ...
    #存取最好的一次，記得修改檔案路徑
    filepath = "AIOT_CNN_LSTM_model.h5"
    checkpoint = ModelCheckpoint(filepath=filepath, monitor='accuracy', verbose=1, save_best_only=True,
mode='max')
